...
import { p as parse, e as expandGlob, b as builtins, N as NPM_REGISTRY_URL_SLASH, V as VERSIONS, o as ok, r as resolve, f as fail, a as relative, G as GREEN, R as RESET, C as CYAN, c as createHash, d as proxiedFetch, D as DEFAULT_ENV, E as EventEmitter, M as MOCK_PID, W as Writable, g as Readable, h as createProcessContext, s as setActiveContext, S as ScriptEngine, i as isExitSentinel, j as format, k as getAllServers, P as ProcessExitSentinel, l as getActiveContext, m as getRegistry, n as closeAllServers, q as resetActiveInterfaceCount, B as Buffer2, __tla as __tla_0 } from "./index-Bc-NjotT.js";
let ShellProcess, clearStreamingCallbacks, child_process, exec, execFile, execFileSync, execSync, executeNodeBinary, fork, getShellCwd, handleIPCFromParent, initShellExec, isStdinRaw, notifyTerminalResize, sendStdin, setForkChildCallback, setIPCReceiveHandler, setIPCSend, setSabEnabled, setShellCwd, setSpawnChildCallback, setStreamingCallbacks, setSyncChannel, shellExec, spawn, spawnSync;
...
`,
    ...
    async execCommand(cmd, stdin) {
      ...
      if (expandedArgs[0] === "sh" && expandedArgs[1] === "-c" && expandedArgs[2]) {
        return this.exec(expandedArgs[2], {
          cwd: this.cwd,
          env: this.env
        });
      }
    ...
    for (const c of commits) {
      if (format2 !== null) {
        let line = format2.replace(/%H/g, c.hash).replace(/%h/g, c.hash.slice(0, 7)).replace(/%s/g, c.message.split("\n")[0]).replace(/%an/g, c.author.split(" <")[0]).replace(/%ae/g, (c.author.match(/<(.+?)>/) ?? [
          "",
          ""
        ])[1]).replace(/%d/g, c.hash === head && currentBranch ? ` (HEAD -> ${currentBranch})` : "").replace(/%n/g, "\n");
        out += line + "\n";
      } else if (oneline) {
...
`);
  ...
  notifyTerminalResize = function(cols, rows) {
    var _a, _b, _c, _d;
    for (const p of _activeProcs) {
      (_b = (_a = p.stdout) == null ? void 0 : _a._setSize) == null ? void 0 : _b.call(_a, cols, rows);
      (_d = (_c = p.stderr) == null ? void 0 : _c._setSize) == null ? void 0 : _d.call(_c, cols, rows);
    }
  };
  ...
  setSabEnabled = function(enabled) {
    _sabEnabled = enabled;
  };
...
`;
  ...
  async function installPackages(args, ctx, pm = "npm") {
    const { DependencyInstaller } = await import("./index-Bc-NjotT.js").then(async (m) => {
      await m.__tla;
      return m;
    }).then((n) => n.aj);
...
`;
    ...
    try {
      const { RegistryClient } = await import("./index-Bc-NjotT.js").then(async (m) => {
        await m.__tla;
        return m;
      }).then((n) => n.ax);
...
`;
    ...
    localCtx.stdoutSink = (prevCtx == null ? void 0 : prevCtx.stdoutSink) ?? _stdoutSink;
    localCtx.stderrSink = (prevCtx == null ? void 0 : prevCtx.stderrSink) ?? _stderrSink;
    localCtx.liveStdin = (prevCtx == null ? void 0 : prevCtx.liveStdin) ?? _liveStdin;
    localCtx.termCols = (prevCtx == null ? void 0 : prevCtx.termCols) ?? _termCols;
    localCtx.termRows = (prevCtx == null ? void 0 : prevCtx.termRows) ?? _termRows;
    if (prevCtx && prevCtx.abortController.signal.aborted) {
      localCtx.abortController.abort();
    } else if (_haltSignal == null ? void 0 : _haltSignal.aborted) {
      localCtx.abortController.abort();
    } else if (_haltSignal) {
      _haltSignal.addEventListener("abort", () => localCtx.abortController.abort(), {
        once: true
      });
    }
    ...
    try {
      ...
      if (isFork) {
        ...
        proc.disconnect = () => {
          ...
          ipcHandle == null ? void 0 : ipcHandle.close();
          ipcHandle = null;
        };
      }
...
`;
      ...
      try {
        ...
        while (!didExit && !(myHaltSignal == null ? void 0 : myHaltSignal.aborted)) {
          if (!tlaSettled) {
            const racers2 = [
              tlaDonePromise,
...
`,
  ...
  execSync = function(cmd, opts) {
    ...
    if (!_syncChannel) {
      throw new Error("[Nodepod] execSync needs SharedArrayBuffer + SyncChannel. enable COOP/COEP headers, or drop `enableSharedArrayBuffer: false` from NodepodOptions.");
    }
...
`);
    ...
    if (_spawnChildFn) {
      ...
      _spawnChildFn(command, spawnArgs, {
        ...
        onStdout: (data) => {
          var _a;
          stdoutStreamed = true;
          if (stdioArr[1] === "pipe") (_a = child.stdout) == null ? void 0 : _a.push(Buffer2.from(data));
          if (stdoutInherit) {
            const sink = getStdoutSink();
            if (sink) sink(data);
          }
        ...
        onStderr: (data) => {
          var _a;
          stderrStreamed = true;
          if (stdioArr[2] === "pipe") (_a = child.stderr) == null ? void 0 : _a.push(Buffer2.from(data));
          if (stderrInherit) {
            const sink = getStderrSink();
            if (sink) sink(data);
          }
  ...
  spawnSync = function(cmd, args, opts) {
    ...
    if (!_syncChannel) {
      throw new Error("[Nodepod] spawnSync needs SharedArrayBuffer + SyncChannel. enable COOP/COEP headers, or drop `enableSharedArrayBuffer: false` from NodepodOptions.");
    }
    const slot = _syncChannel.allocateSlot();
